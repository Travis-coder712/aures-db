...
**Latest shipped version:** v3.32.0 — Deep-dive extraction (28 v3.31.0 rows enriched with 78 new structural fields — CF/GWh/noise/setback for wind, inverter/cell/PCS for hybrid BESS) + new **Grid Connection Headroom** intelligence page (`/intelligence/grid-connections`) surfacing the ElectraNet SA data with region tiles, existing connections tables, forward-looking capacity, bay counts, and RAS. Preceded by v3.31.0 (hybrid + wind tranche 2 + VIC AEMO register + ElectraNet SA connections). Three parallel research agents extracted (a) 30 hybrid solar+BESS projects (hybrid coverage 0 → 24.8%) and (b) 33 large wind farms (wind coverage 31 → 64, 14% → 29.8%). Fourth agent parsed the ElectraNet Network Connection Opportunities to 2030 PDF into structured JSONs (76 existing + 62 available + 113 bays + 24 RAS rows). New `pipeline/importers/import_aemo_gen_register.py` ingested the AEMO VIC NER 5.18A.2 register (41 rows, 100% match rate for renewable rows, retired thermal correctly excluded). New `aemo_gen_register` table (state + snapshot natural key, multi-state ready). Preceded by v3.30.0 (solar backlog closure) and v3.29.0 (AEMO KCI ingest).
...
Landed in v3.32.0:
- **Second-wave deep-dive extraction.** Four parallel research agents worked the 63 v3.31.0 wind + hybrid rows (~16 each). 28 projects received new structural fields (78 fields total). Field distribution: `assumed_annual_energy_gwh` (10), `connection_voltage_kv` (9), `assumed_capacity_factor_pct` (6), `noise_limit_dba` (5), `connection_augmentation` (5), plus targeted BESS fields (`inverter_supplier`, `cell_supplier`, `pcs_type`, `inverter_count`). **Notable finds:** Blind Creek and Bungama both use SMA + Wärtsilä Gridsolv Quantum HE combo (identical BoP hybrid pattern); Bundey Genaspi confirms grid-forming PCS at scoping stage; Capital Wind Farm's design CF (35.8%) vs actual (27.88%) is an unusually large forecast miss; Cattle Creek plans up to 143 WTGs at 270 m tip height. Batches 3+4 (older pre-2013 permit wind farms) yielded thin structural data — Wikipedia + developer sites don't inline EIS-era detail. Applied via `scratchpad/apply_deepdive.py` (direct UPDATE against `eis_technical_specs` + JSON patch — bypasses re-running the full enricher).
- **Grid Connection Headroom intelligence page** (`/intelligence/grid-connections`). New React component surfacing the v3.31.0 ElectraNet ingest with 3 region tiles (Mid North / Eyre Peninsula / South East), per-region tables for existing/committed connections, forward-looking gen+load capacity per substation (with proponent-interest chips), available bays per substation, and RAS callouts. Footer placeholders reference AEMO KCI for NSW/QLD/VIC/TAS. Complementary lens to the KCI-focused EIS Technical page. New `ElectranetExisting` / `ElectranetAvailable` / `ElectranetBay` / `ElectranetRAS` / `ElectranetManifest` types. New `fetchElectranetData` dataService function (bundled fetch of manifest + 4 JSONs). Registered in IntelligenceHub. **Headroom finding: SA has 13.6 GW of forward-looking generation headroom (Mid North 6,445 MW + Eyre Peninsula 2,640 MW + South East 4,575 MW), with Bundey 275 kV, Tungkillo 275 kV, and South East 275 kV the largest single-substation opportunities.**

Landed in v3.31.0:
- **Hybrid solar+BESS source-hunt.** 30 highest-priority hybrids (1 operating + 4 construction + 25 top-MW development) run through a single research agent. 27 credible sources found + 3 nulls (Glenroy Hybrid Facility, Sturt Solar, Farmers Creek Energy Park — all AEMO KCI-only registrations with no public planning docs). Bundled into `pipeline/enrichers/enrich_eis_specs.py` as hybrid entries. **Hybrid coverage: 0/121 → 30/121 (24.8%).** Rich per-project data: solar MWac, wind MWac (for solar+wind+BESS), BESS MW / MWh / duration, planning regime, connection point, NSP, developer, current status, CIS/LTESA linkage where known.
- **Wind tranche 2.** All 33 operating + construction wind farms ≥100 MW that lacked spec rows after tranche 1. Rich per-project data: turbine model, count, hub height, rotor diameter, connection point, NSP, notes. VIC pre-2013 wind farms confirmed under Ministerial Planning Permit (Planning & Environment Act 1987) — no formal EES. Wikipedia + power-technology.com + developer sites converged on the same turbine model/count/hub-height for every farm. **Wind coverage: 31/215 → 64/215 (29.8%).**
- **ElectraNet SA connections registry.** Parsed `Network Connection Opportunities to 2030 (FY25-26, July 2026)` — 62 pages, public v1.0. Extracted 4 datasets to `frontend/public/data/electranet/`: `existing.json` (76 committed generation + load rows across Mid North / Eyre Peninsula / South East), `available.json` (62 per-substation available capacity rows), `bays.json` (113 per-substation available bay rows), `ras.json` (24 Remedial Action Scheme rows). Plus a manifest. **This replaces the ElectraNet Connections Report as the SA connection-registry source** and is the analytical equivalent of AEMO KCI for SA — but forward-looking (headroom per substation, not per-project applications).
- **AEMO VIC register (NER 5.18A.2) ingest.** New `pipeline/importers/import_aemo_gen_register.py` parses the AEMO per-state register PDF (multi-state ready via state + snapshot natural key). Ingested 41 rows, matched 29 to AURES projects (100% match rate for in-scope renewable rows). 12 unmatched are all thermal / retired coal (Angelsea, Energy Brix, Hazelwood, Jeeralang A/B, Laverton North, Loy Yang A/B, Mortlake, Newport, Valley Power, Yallourn W — correctly outside AURES scope). Tech-guard prevents cross-tech false positives (retired Hazelwood coal plant no longer wrongly linked to Hazelwood BESS). Retirement dates flagged for 3 assets. **Note:** This register is what Travis assumed was the AusNet TCPR — it's actually AEMO's transmission-connected register under NER 5.18A.2. VicGrid does not appear to publish a per-DNSP TCPR equivalent.
- **DB backup preserved** at `database/aures.db.bak-2026-08-09-pre-v3-31` (4.0 GB).
- **Regex catastrophic-backtracking learning:** first parser used lazy `.+?` quantifiers separated by `\s{2,}` — this hangs on lines that don't match. Fixed to `re.split(r'\s{2,}', line)` + right-scan for MW / date tokens (no backtracking risk).
...
### Landed — shipped as v3.23.0 and v3.23.1
...
### Landed in v3.23.1 (2026-07-26)

- **AEMO apr-2026 ingest** — new `--snapshot 2026-04` flag on importer; both `2026-01` and `2026-04` snapshots coexist idempotently. 30 new pipeline projects (Futcher 400 MW, Middle Hut 800 MW, Tuggerah 500 MW, etc). 208 project CODs refreshed via enrich_from_aemo now that MAX() sees both snapshots.
- **News + WattClarity** — added as 4th feed source. `news_articles` 410 → 500, all sources fresh through 2026-07-24.
- **schema.sql regenerated** from live DB as honest snapshot (was missing ~14 tables).
- **Dropped `projects.development_score`** — NULL for all rows; no code populated it. References removed from exporter, types.ts, sample-projects.ts, guides.ts.
- **`data_confidence='confirmed'` reconciled** — 5 rows + 21 overlays migrated to `high`.
- **`timeline_events` partial UNIQUE index** — DB-level backstop for the audit follow-ups.
- **`import_energyco_rez_access.py` hardened** — replaced fragile `detail LIKE` dedup with `data_source='energyco:<id>'` + `ON CONFLICT DO NOTHING`.
- **Pre-existing quartile bug fixed** — `_quartile_from_score` was raising `ValueError` on floating-point mismatch between cohort and record score.

### Still open

- **`import_dispatchload.py`** and **`import_nemweb_bids.py`** use `INSERT OR IGNORE` (first-write-wins) — safe from dupes but won't refresh a changed source row. Fine for immutable historical dispatch data; risk only if AEMO retroactively revises a row. Not fixed — deliberate deferral.
- **`harvest_facility_metadata.py`** — Python-level SELECT-first dedup for timeline_events. Now sits behind the new DB-level partial UNIQUE index as a backstop; further hardening not urgent.
